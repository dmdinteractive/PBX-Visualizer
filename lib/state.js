...
export const EXTERNAL = 'EXTERNAL'; // sentinel party id for anything outside our extensions
...
export class CallState extends EventEmitter {
  constructor(stations = []) {
    ...
    this.calls = new Map(); // callId -> { id, fromId, toId, fromLabel, toLabel, state, external, since }
  ...
  // Register a station we didn't know about (e.g. a new extension seen live).
  ...
  statusOf(stationId) {
    ...
    for (const c of this.calls.values()) {
      if (c.fromId !== stationId && c.toId !== stationId) continue;
